  ...
  if (!container) {
    container = {
      ...
      trigger: function trigger(event) {
        var handler = container && container.handlers && container.handlers[event];
        if (!handler) return false;
        var title = 'container handler(' + event + '-' + (container.displayName || '') + '):';
        try {
          for (var _len = arguments.length, args = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
            args[_key - 1] = arguments[_key];
          }

          app.verbose.apply(app, [title].concat(args));
